...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', async function (assert) {
        ...
        // GSM-7
        await testUtils.fields.editAndTrigger(this.target.querySelector('.o_input'), "Hello from Webkul", 'input');
        assert.strictEqual(this.target.querySelector('.o_sms_count').textContent, '15 characters, fits in 1 SMS (GSM7) ',
            'Should be "15 characters, fits in 1 SMS (GSM7) " for "Hello from Webkul"');
        // GSM-7 with \n => this one count as 2 characters
        await testUtils.fields.editAndTrigger(this.target.querySelector('.o_input'), "Hello from Webkul\n", 'input');
        assert.strictEqual(this.target.querySelector('.o_sms_count').textContent, '17 characters, fits in 1 SMS (GSM7) ',
            'Should be "17 characters, fits in 1 SMS (GSM7) " for "Hello from Webkul\\n"');
        // Unicode => ê
        await testUtils.fields.editAndTrigger(this.target.querySelector('.o_input'), "Hêllo from Webkul", 'input');
        assert.strictEqual(this.target.querySelector('.o_sms_count').textContent, '15 characters, fits in 1 SMS (UNICODE) ',
            'Should be "15 characters, fits in 1 SMS (UNICODE) " for "Hêllo from Webkul"');
        // GSM-7 with 160c
