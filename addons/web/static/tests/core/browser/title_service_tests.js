...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Metatron" });
    assert.strictEqual(env.services.title.current, "Metatron");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Metatron", chat: null });
    assert.strictEqual(env.services.title.current, "Metatron");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Metatron - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Metatron", action: "Import" });
    assert.strictEqual(env.services.title.current, "Metatron - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Metatron", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Metatron - Import"); // parts is a copy!
});
